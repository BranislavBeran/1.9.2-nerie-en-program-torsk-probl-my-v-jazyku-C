...
char circ_circ_inner_tangents(CIRCLE *c1, CIRCLE *c2, LINE *t1, LINE *t2){
	...
	int dot;
	POINT a,b,c;
	float vzdialenost,l,d,uhol,vx1,vy1,vx2,vy2,vektorssX,vektorssY;												//l->vzdialenost stredu kruznice od priesecniku kruznic, vzdialenost ->vzdialenost stredov kruznic,d->strana pravouhleho trojuhokniku
	...
	if(((vzdialenost<(c1->r+c2->r))&&(vzdialenost!=abs(c1->r-c2->r)))||vzdialenost<abs(c1->r-c2->r))	dot=0;	//v pripade ak nemaju ziadne vnutorne dotycnice

	else if(vzdialenost==(c1->r+c2->r)){  																		//v pripade ak maju 1 vnutornu dotycnicu jedna kruznica nelezi v druhej
		dot=1; 	
		
		a.x=c1->c.x+(c1->r/(c1->r+c2->r))*(vektorssX);
		a.y=c1->c.y+(c1->r/(c1->r+c2->r))*(vektorssY);															//bod dotiku kruznic
	...
	else if(vzdialenost==abs(c1->r-c2->r)){																		//v pripade ak maju 1 vnutornu dotycnicu jedna kruznica lezi v druhej
		dot=1;
...
main(){
	...
	if(r1==0||r2==0) printf("zle zadany polomer");
